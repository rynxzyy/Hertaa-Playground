...
require('../settings.js');
const { jidNormalizedUser, extractMessageContent, downloadMediaMessage, proto, areJidsSameUser, generateWAMessage } = require('baileys');
...
module.exports = async (messages, sock, store) => {
    ...
    if (messages.key) {
        ...
        m.sender = jidNormalizedUser(
            m.fromMe ? sock.user.id : m.isGroup ? m.participant : m.chat,
        );
    }
    if (m.isGroup) {
        ...
        if (m.msg?.contextInfo?.quotedMessage) {
            ...
            if (m.quoted.message) {
                ...
                if (m.quoted.isMedia) {
                    ...
                    m.quoted.copy = (txt) => {
                        return sock.cMod(m.chat, m.quoted, txt);
                    };
                    m.quoted.forward = (Boolean) => {
                        return sock.copyNForward(m.chat, m.quoted, Boolean);
                    };
                }
            }
        }
    ...
    if (m.isMedia) {
        ...
        m.copy = (txt) => {
            return sock.cMod(m.chat, m, txt);
        };
        m.forward = (Boolean) => {
            return sock.copyNForward(m.chat, m, Boolean);
        };
    }
