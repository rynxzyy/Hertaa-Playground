...
class Scraper {
    ...
    load = async () => {
        let data = await Scandir('./lib/scrape/src');
        for (let i of data) {
            ...
            try {
                ...
                console.log(chalk.bgHex('#e74c3c').bold(` Failed to load Scraper : ${e} `));
    ...
    watch = async () => {
        ...
        watcher.on('add', async (filename) => {
            ...
            this.#src[name] = require(filename);
            console.log(chalk.bgHex('#90EE90').hex('#333').bold(` New Scraper has been added : ${name} `));
            return this.load();
        });
        watcher.on('change', (filename) => {
            ...
            console.log(chalk.bgHex('#90EE90').hex('#333').bold(` Scraper has been changed : ${name} `));
            return this.load();
        });
        watcher.on('unlink', (filename) => {
            ...
            let name = filename.split('/').pop().replace('.js', '');
            delete this.#src[name];
            console.log(chalk.bgHex('#90EE90').hex('#333').bold(` Scraper has been removed : ${name} `));
            return this.load();
        });
    };
